...
|-------------:|---------:|---------:|---------:|----------------:|-------------:|
| 0            | 7        | 9        | 错误      | 19,432,734,720 (≈18532.5 MB) | 63.9386 |

补充说明（来自 MP-SPDZ 运行日志）：

1. Party 0 发送：`9238.53 MB`
2. Party 1 发送：`9293.94 MB`
3. 虚拟机轮数：`~1,438,146 rounds`
...
1. **FC2 在 MPC 下通信开销明显偏大**：单样本推理总通信约 `18.5 GB`，主要来自安全乘法与比较操作（日志显示需要约 `5.9M` triples 和 `1.4M` 轮）。
2. **明文精度与 MPC 精度未完全对齐会影响预测正确性**：本次明文训练准确率达到 `97.73%`，但 sample 0 在 MPC 下预测错误。一个高概率原因是固定点导出使用 `fractional_bits=16`，而 `fc2_mnist_infer.mpc` 中 `sfix.set_precision(32, 32)` 设置了 32 位小数，存在尺度不一致风险。
...
5. **下一步优化方向**：
   - 将导出与 MPC 的固定点精度统一（例如都使用 16 位小数，或都使用 32 位小数）。
   - 按日志建议启用 `program.use_trunc_pr = True` 和 `program.use_split(2)`，尝试降低运行时。
   - 对多个测试样本批量评估，给出 MPC 侧真实准确率与均值开销。

## 实验总结

本次实验已完整跑通了从训练、参数导出、输入准备到两方 MPC 推理的 baseline 闭环，并获得了可复现实验日志。结果显示，私有推理链路可执行，但在当前参数设置下通信成本较高，且 sample 级别预测与明文模型存在偏差，提示固定点精度与 MPC 配置仍需进一步对齐。ResNet‑50 部分目前主要验证了编译与执行流程，后续需要补全前向计算后再进行准确率与开销评估。
